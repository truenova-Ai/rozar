...
import {
  ChevronLeft, Upload, AlertCircle, Loader2,
  CheckCircle2, XCircle, ExternalLink, Search, Newspaper, Globe,
} from 'lucide-react'
...
const WHAT_TO_DO = {
  FAKE: [
    { icon: '🚫', text: 'Do NOT share this image — spreading misinformation causes real harm.' },
    { icon: '🔍', text: 'Search the extracted headline on Google News or Reuters to find verified coverage.' },
    { icon: '✅', text: 'Check Snopes.com or PolitiFact.com — professionals may have already fact-checked this.' },
    { icon: '🖼️', text: 'Use Google Reverse Image Search to check if the image is misused from another context.' },
    { icon: '👤', text: 'Look for named reporters, official institutions, and verifiable quotes in the image.' },
  ...
  REAL: [
    ...
    { icon: '📖', text: 'Find and read the full original article before sharing.' },
    { icon: '🔗', text: 'Cross-reference with another major outlet (BBC, Reuters, AP) to confirm.' },
    { icon: '📅', text: 'Check the publication date — even real news can mislead when recycled.' },
    { icon: '📌', text: 'Include the source name when sharing so others can verify.' },
...
export default function OCRAnalysis({ onBack, onAddHistory }: OCRAnalysisProps) {
  ...
  const fetchRelated = (text: string) => {
  ...
  // ── Step 1: Upload image → call OCR API ──
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.verdict && data.verdict !== 'UNKNOWN' && data.extracted_text) {
        const r = {
        ...
        fetchRelated(data.extracted_text)
      } else if (!data.extracted_text) {
        setError(data.explanation || 'No text found in image.')
      }
    } catch (err) {
  ...
  // ── Step 2 (optional): Re-analyze edited text ──
    ...
    <div className="w-full min-h-screen bg-white dark:bg-black p-6 md:p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <p className="text-gray-600 dark:text-gray-400 text-base font-light mb-12">
          Upload images to extract and verify content authenticity
        </p>
              ...
              {!image ? (
                <div className="relative">
                  ...
                  <label
                    ...
                    className="flex flex-col items-center justify-center p-12 bg-white dark:bg-white/5 border-2 border-dashed border-gray-300 dark:border-white/20 rounded-2xl cursor-pointer hover:border-gray-400 dark:hover:border-white/30 hover:bg-gray-50 dark:hover:bg-white/[7%] transition-all duration-200 backdrop-blur-sm"
                  >
                    <Upload size={40} className="text-gray-400 dark:text-gray-600 mb-4" strokeWidth={1.5} />
                    <p className="text-base font-light text-gray-700 dark:text-gray-300 mb-2">Drop an image here, or click to browse</p>
                    <p className="text-sm text-gray-500 dark:text-gray-500 font-light">PNG, JPG, WEBP</p>
                  </label>
                </div>
              ) : (
                <>
                  <div className="mb-6 rounded-2xl overflow-hidden bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10">
                    <img src={image} alt="Uploaded" className="w-full max-h-96 object-contain p-4" />
                  </div>
                  {isExtracting && (
                    <div className="flex items-center gap-3 mb-4 text-gray-600 dark:text-gray-400 text-sm font-light">
                      <Loader2 size={16} className="animate-spin" />
                      Extracting text with OCR…
                    </div>
                  )}
                  {!isExtracting && (
                    <button onClick={resetForm} className="text-sm text-gray-600 dark:text-gray-500 hover:text-black dark:hover:text-white transition-colors font-light">
                      Change image
                    </button>
                  )}
                </>
            ...
            {extractedText && !isExtracting && (
              <div className="mb-8">
                <label className="block text-gray-700 dark:text-gray-300 font-light text-sm mb-4">
                  Extracted text — you can edit before re-analyzing
                </label>
                <textarea
                  value={extractedText}
                  onChange={(e) => setExtractedText(e.target.value)}
                  className="w-full h-40 p-5 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl text-black dark:text-white placeholder-gray-400 dark:placeholder-gray-600 focus:outline-none focus:border-gray-400 dark:focus:border-white/20 transition-all duration-200 font-light resize-none backdrop-blur-sm"
                />
        ...
        <div className="space-y-5">
          ...
          {/* Image Preview */}
          {image && (
            <div className="p-4 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl backdrop-blur-sm">
              <img src={image} alt="Analyzed" className="w-full max-h-64 object-contain rounded-lg" />
              {result.ocr_method && (
                <p className="text-xs text-gray-500 dark:text-gray-600 font-light mt-2 text-center">
                  OCR engine: {result.ocr_method}
                </p>
              )}
            </div>
          )}
          ...
          <div className="p-6 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl backdrop-blur-sm">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-600 font-light mb-4">
              Search for this image's topic on trusted fact-checking and news platforms:
            </p>
          ...
          {/* ── Extracted Text ── */}
          <div className="p-6 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl backdrop-blur-sm">
            <p className="text-xs text-gray-600 dark:text-gray-500 font-light mb-3">EXTRACTED TEXT</p>
